...
# Линейная регрессия подгоняет данные к прямой линии, пытаемся установить линейную связь между переменными и предсказать новые данные.
...
w_0 = (
    sum(y[i] for i in range(n)) / n 
    - w_1 * (sum(x[i] for i in range(n)) / n)
)
...
w = inv(x_l.transpose() @ x_l) @ (x_l.transpose() @ y)
